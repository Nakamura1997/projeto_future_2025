    ...
    def lgpd_score(self, request):
        ...
        try:
            ...
            for category in categories:
                cat_responses = user_responses.filter(
                    question__category__startswith=category
                )
        ...
        try:
            if isinstance(request.data, list):
                ...
                existing = AssessmentResponse.objects.filter(
                    user=request.user, question__in=question_ids
...
class ExecutiveReportView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            response_count = AssessmentResponse.objects.filter(
                user=request.user
            ).count()
            question_count = Question.objects.count()
            completion_percentage = (
                (response_count / question_count) * 100 if question_count > 0 else 0
            )
...
class SaveAssessmentView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            user_responses = AssessmentResponse.objects.filter(
                user=request.user
            ).count()
